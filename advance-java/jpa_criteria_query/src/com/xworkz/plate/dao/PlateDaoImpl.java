...
public class PlateDaoImpl implements PlateDao {
	...
	public void save(PlateEntity entity) {
		...
		try {
			...
			//CriteriaQuery:- it is the type-safe in terms of referencing attributes or retriving attributes.
			//root:- roots are define the basic from which joins, paths and attributes are available in the query
		
			
			
			
			//Root<PlateEntity> root=session.getCriteriaBuilder().createQuery(PlateEntity.class).from(PlateEntity.class);
